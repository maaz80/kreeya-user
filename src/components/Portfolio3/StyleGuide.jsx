          ...
          <section className="for-white-icons bg-white py-20 plus-jakarta-sans">
               ...
               <div className="relative grid grid-cols-3 w-full">
                    ...
                    {colors.map((color) => (
                         <div
                              key={color}
                              className="h-65.5 md:h-80 lg:h-202.75 flex items-start justify-center pt-10 text-dark-black md:font-bold text-[12px] md:text-[32px]"
                              style={{ background: color }}
                         >
                              {color}
                         </div>
                    ))}
                    <div className="absolute -bottom-5 w-[65%] lg:w-[85%] left-1/2 -translate-x-1/2 bg-white rounded-2xl p-2 md:p-5 lg:p-10 ">
                         <div className="rounded-2xl overflow-hidden grid grid-cols-4">
                              {semicolors.map((color) => (
                                   <div
                                        key={color}
                                        className="h-15 md:h-30 lg:h-80 flex items-start justify-center pt-10 text-dark-black md:font-bold text-[12px] md:text-[32px]"
                                        style={{ background: color }}
                                   >
